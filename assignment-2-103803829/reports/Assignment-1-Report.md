...
For migration from Hot to Cold, a daily service could run, that selects the tables from the days that are old enough and exports them to the cold storage, after which it delets them from the hot one. The partitioning makes the moving of data safe, because entire partitions are moved at once. It can be a scheduled batch job (possibly using Airflow). We need to use record counts and checksums for integrity checks of migrated data.
